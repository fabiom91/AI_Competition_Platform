...
function showhide_newcomment() {
  alert("This function has been temporarily disabled. For questions and support, please contact support@your_support_email.com")
  // if (checkLogin()==false){
  ...
  function generate_chart(){
    ...
    Array.prototype.forEach.call(charts, function(canvas) {
      ...
      var all_scores = Object.values(last_scores).concat(Object.values(best_scores));
      ...
      var myRadarChart = new Chart(ctxR, {
        ...
        options: {
          responsive: false,
          // Every chart must share one axis, otherwise Chart.js scales each radar to its
          // own data and a lower score can be plotted further from the centre than a
          // higher one on the chart next to it.
          scale: {
            ticks: {
              beginAtZero: true,
              max: max_value
    ...
    if (confirm_delete == true) {
      $.post('/delete',{'uid':authorID,'aid':AID,'id':id}).done(function(re){
        if (re['response'] == 'comment_deleted'){
          ...
          alert('This competition has already a submission. To delete your competition, contact admin at support@your_support_email.com');
        }
      });
    }
  }
